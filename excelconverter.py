...
# Define mapping of DAT file columns to Excel template columns
column_mapping = {
    "EmpID": "Employee ID",
    "Timestamp": "Time Log",
    "Dept": "Department",
...
def convert_dat_to_excel_with_mapping():
    # Select the DAT file
    dat_file = filedialog.askopenfilename(filetypes=[("DAT Files", "*.dat")])
    if not dat_file:
        return

    # Load the DAT file (assuming tab-separated)
    df = pd.read_csv(dat_file, delimiter="\t", dtype=str)

    # Ensure the DAT file has expected columns
    if not all(col in df.columns for col in column_mapping.keys()):
        messagebox.showerror("Error", "DAT file does not contain expected columns.")
        return

    # Rename DAT file columns to match the Excel template
    df.rename(columns=column_mapping, inplace=True)

    # Load the predefined Excel template
    excel_template = filedialog.askopenfilename(filetypes=[("Excel Files", "*.xlsx")])
    if not excel_template:
        return

    wb = load_workbook(excel_template)
    ws = wb.active  # Assuming data goes to the first sheet

    # Write data to the template (starting from row 2, after headers)
    ...
    save_path = filedialog.asksaveasfilename(defaultextension=".xlsx",
                                             ...
                                             title="Save Mapped Excel File")

    if save_path:
        wb.save(save_path)
        messagebox.showinfo("Success", "DAT file successfully converted using the template!")
